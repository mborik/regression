...
if (process.argv.length < 3)
	return console.error('few arguments, run "node stvorcek2code.js {export.ase}"!');
...
let log = `;\t\t\t bc\t de\n`;
let src = fs.readFileSync(process.argv[2], { encoding: 'ascii' });
let matches, frames = [];
...
const logfile = process.argv[2].replace(/\.ase$/, '.inc');
fs.writeFileSync(logfile, log, { flag: 'w' });
